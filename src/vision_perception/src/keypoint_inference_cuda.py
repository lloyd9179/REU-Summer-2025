...
# Utility function to load ONNX model into TensorRT engine
def build_engine(onnx_file_path, max_batch_size=1, max_workspace_size=1 << 30):
    logger = trt.Logger(trt.Logger.WARNING)
    builder = trt.Builder(logger)
    # network = builder.create_network(common_flags=1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH))
    ...
    # builder.fp16_mode = True  # Enable FP16 if supported
...
class Perception(Node):
    ...
    def __init__(self, trt_engine):
        ...
        self.trt_engine = trt_engine  # TensorRT engine
        ...
        self.declare_parameter('output_heading', '/hunter/vision/heading')
        self.declare_parameter('output_distance', '/hunter/vision/distance')
        ...
        output_distance_topic = self.get_parameter_or('~output_distance','/hunter/vision/distance')
        self.output_distance = self.create_publisher(Float32MultiArray,output_distance_topic, qos_profile = 1) 

        output_heading_topic = self.get_parameter_or('~output_heading', '/hunter/vision/heading') 
        self.output_heading = self.create_publisher(Float32MultiArray,output_heading_topic, qos_profile = 1) 

        self.output_vp = self.create_publisher(Float32MultiArray,'/hunter/vision/vp', qos_profile = 1) 
        self.output_ll = self.create_publisher(Float32MultiArray,'/hunter/vision/ll', qos_profile = 1) 
        self.output_lr = self.create_publisher(Float32MultiArray,'/hunter/vision/lr', qos_profile = 1) 

        self.output_vp_conf = self.create_publisher(Float32MultiArray,'/hunter/vision/vp_confidence', qos_profile = 1) 
        self.output_ll_conf = self.create_publisher(Float32MultiArray,'/hunter/vision/ll_confidence', qos_profile = 1) 
        self.output_lr_conf = self.create_publisher(Float32MultiArray,'/hunter/vision/lr_confidence', qos_profile = 1) 

        output_vpl_img_topic = self.get_parameter_or('~output_vpl_img','/hunter/vision/vpl_img_argmax')
        self.output_vpl_argmax_img = self.create_publisher(Image,output_vpl_img_topic, qos_profile=1)
        self.output_vpl_img = self.create_publisher(Image,'/hunter/vision/vpl_img', qos_profile=1)
    ...
    def subscribers(self):
        ...
        input_topic_compressed = '/front_cam/image_compressed'
        ...
        self.img_subscriber_compressed = self.create_subscription(CompressedImage,input_topic_compressed, self.callback_compressed,qos_profile = qos_profile)  
    ...
    def processImage(self, ros_data, msg=''):
        if self.enable_perception == False:
            print("returned false perception")
            return -1  
        ...
        try:
            #Using a raw image takes more time to process
            #the below method takes less time to convert into cv_image
            msg = ros_data
            # # print("MSG:::", msg)
            ...
            #the below method takes less time to convert into cv_image
            # Convert the ROS CompressedImage to a NumPy array
            np_arr = np.frombuffer(msg.data, np.uint8)  # Convert compressed byte stream to NumPy array

            # Decode the image from the byte array
            image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)  # This decodes the image into an OpenCV format (BGR)

            if image is None:
                raise ValueError("Could not decode the compressed image")
        ...
        fx,fy,cx,cy = self.flx,self.fly,self.pfx,self.pfy
        ...
        # Prepare output buffers (adjust shape according to the output of your model)
        output_shape = (3,56,80)  # Example output shape, adjust to match your model
        ...
        #the below code is for visualisation of the lane detection, you can comment it to improve inference time. Uncomment for debugging.
        ...
        # self.get_logger().info("times:  "+str(mid1-start_time)+', ' +str(mid2-mid1)+', '+str(end_time-mid2))
...
def main(args=None):
    ...
    trt_engine = build_engine(dir_path)
